...
module.exports = function (session) {
  ...
  function RedisStore (options) {
    ...
    if (options.secondaryUrl) {
       options.secondarySocket = options.secondaryUrl;
    }
    ...
    if(options.secondarySocket){
        this.secondaryClient = redis.createClient(options.secondarySocket, options);
    }
    // logErrors
    if(options.logErrors){
      ...
      if(this.secondaryClient){
          this.secondaryClient.on('error', options.logErrors);
      }
    }
  ...
  RedisStore.prototype.get = function (sid, fn) {
    ...
    store.client.get(psid, function (er, data) {
      if (er) return fn(er);
      if (!data) {
        // try to find in secondary
          if(store.secondaryClient){
              store.secondaryClient.get(psid,function(ser,sdata){
                if(!sdata){
                  return fn();
                }
                else{
                    ...
                    args.push(sdata);
                    store.client.set(args);

                    debug('GOT %s', sdata);
                    var sresult;
                    try {
                        sresult = store.serializer.parse(sdata);
                    }
                    catch (er) {
                        return fn(er);
                    }
                    return fn(null, sresult);
                }
              });
          }
          else{
